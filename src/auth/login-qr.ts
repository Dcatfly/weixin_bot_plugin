...
export async function waitForWeixinLogin(opts: {
  ...
  apiBaseUrl: string;
  botType?: string;
  onQrRefresh?: (newQrUrl: string) => void;
  ...
  while (Date.now() < deadline) {
    try {
      ...
      switch (statusResponse.status) {
        case "wait":
          if (opts.verbose) {
            logger.debug("QR status poll...");
          }
          break;
        case "scaned":
          if (!scannedPrinted) {
            logger.info("已扫码，在微信继续操作...");
            scannedPrinted = true;
          }
          break;
        case "expired": {
          ...
          logger.warn(`二维码已过期，正在刷新...(${qrRefreshCount}/${MAX_QR_REFRESH_COUNT})`);
          logger.info(
